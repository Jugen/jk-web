...
public class ManagedBeanConfigHandler implements ConfigAnnotationHandler {
	...
	static {
		Collection<Class<? extends Annotation>> handles = new ArrayList<Class<? extends Annotation>>(2);
	...
	/**
	 * @see com.sun.faces.application.annotation.ConfigAnnotationHandler#collect(Class,
	...
	private ManagedBeanInfo getBeanInfo(Class<?> annotatedClass, ManagedBean metadata) {
		...
		if (!annotatedFields.isEmpty()) {
			...
			for (Map.Entry<String, Field> entry : annotatedFields.entrySet()) {
				...
				ManagedBeanInfo.ManagedProperty propertyInfo = new ManagedBeanInfo.ManagedProperty(entry.getKey(), f.getType().getName(),
		 ...
		 * ArrayList<ManagedBeanInfo.ManagedProperty>(annotatedMethods.size()); for
		 * (Map.Entry<String,Method> entry : annotatedMethods.entrySet()) { Method m =
		 * entry.getValue(); ManagedProperty property =
		 * m.getAnnotation(ManagedProperty.class); String alias = property.name(); if
		 * (alias != null && alias.length() == 0) { alias = null; }
		 * ManagedBeanInfo.ManagedProperty propertyInfo = new
		 * ManagedBeanInfo.ManagedProperty(alias, entry.getKey(),
		 * m.getParameterTypes()[0].getName(), property.value(), null, null);
		 * properties.add(propertyInfo); } } }
		 */
